...
const scrollToTopWhatsApp = () => {
  const message = encodeURIComponent(
    "Bonjour, je vous contacte via le site web ArtiVisio concernant vos services.",
  );
  window.open(
    `https://wa.me/22508976737?text=${message}`,
    "_blank",
    "noopener,noreferrer",
  );
};
...
export default function Footer() {
  ...
  const socialLinks = [
    {
      icon: FaLinkedin,
      href: "https://www.linkedin.com/company/artivisioco/?viewAsMember=true",
      label: "LinkedIn",
    },
    { icon: FaWhatsapp, href: "https://wa.me/22508976737", label: "WhatsApp" },
    {
      icon: FaFacebook,
      href: "https://www.facebook.com/profile.php?id=61552513933160",
      label: "Facebook",
    },
  ];
  ...
  const handleSubscribe = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let data;
      try {
        data = JSON.parse(text); // parse seulement si c'est du JSON
      } catch {
